<template>
  <div class="w-full">
    <div class="mb-6">
      ...
      <div class="flex items-start justify-between">
        <div class="text-left">
          ...
          <p class="mt-1 text-sm text-gray-600">Actualiza los datos del producto</p>
        </div>
    ...
    <div class="w-full">
      <div v-if="loadingProduct" class="flex items-center justify-center py-12">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>
        <span class="ml-2 text-slate-600">Cargando producto...</span>
      </div>
      ...
      <div v-else class="bg-white py-8 px-6 shadow sm:rounded-lg">
        <form @submit.prevent="handleSave" class="space-y-6 w-full">
          ...
          <div>
            ...
            <div class="mt-1">
              <input id="name" v-model="formData.name" type="text" required maxlength="200"
                class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500': errors.name }" placeholder="Ej: Auriculares Inalámbricos" />
          ...
          <div>
            ...
            <div class="mt-1">
              <textarea id="description" v-model="formData.description" rows="3" maxlength="1000"
                class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500': errors.description }" placeholder="Describe brevemente el producto..."></textarea>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <div class="mt-1">
                <input id="price" v-model.number="formData.price" type="number" step="0.01" min="0" required
                  class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500': errors.price }" placeholder="0.00" />
            ...
            <div>
              ...
              <div class="mt-1">
                <input id="stock" v-model.number="formData.stock" type="number" step="1" min="0"
                  class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500': errors.stock }" placeholder="0" />
            ...
            <div>
              ...
              <div class="mt-1">
                <select id="category" v-model="formData.categoryId" class="block w-full rounded-md border-gray-300 shadow-sm sm:text-sm py-2 px-3">
          ...
          <div v-if="generalError" class="bg-red-50 border border-red-200 rounded-md p-4">
            <div class="flex">
              <Icon icon="mdi:alert-circle" class="h-5 w-5 text-red-400" />
              <div class="ml-3">
                <h3 class="text-sm font-medium text-red-800">Error al actualizar producto</h3>
                <div class="mt-2 text-sm text-red-700">{{ generalError }}</div>
              </div>
            </div>
          </div>
...
async function loadProduct() {
  ...
  if (!id) {
    generalError.value = 'ID de producto no válido'
    return
  }
...
function validateForm() {
  errors.value = {}
  if (!formData.value.name || !formData.value.name.trim()) errors.value.name = 'El nombre es obligatorio'
  else if (formData.value.name.trim().length < 2) errors.value.name = 'El nombre debe tener al menos 2 caracteres'
  else if (formData.value.name.trim().length > 200) errors.value.name = 'El nombre no puede exceder 200 caracteres'

  if (formData.value.description && formData.value.description.length > 1000) errors.value.description = 'La descripción no puede exceder 1000 caracteres'

  if (formData.value.price == null || isNaN(Number(formData.value.price))) errors.value.price = 'El precio es obligatorio y debe ser un número'
  else if (Number(formData.value.price) < 0) errors.value.price = 'El precio no puede ser negativo'
...
function goBack() { router.push('/adminproductos') }
