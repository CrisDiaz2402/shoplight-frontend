			...
			<div class="flex items-center justify-between h-20">
				...
				<div class="flex items-center gap-6">
					...
					<nav class="hidden md:flex items-center gap-4 text-slate-700">
						<!-- Los enlaces se comentan temporalmente porque no se utilizarán por ahora -->
						<!--
...
function toggleProfile() {
	if (!logged.value) {
		// abrir modal de autenticación en modo login (global)
...
function openCart() {
	// If not logged in, open login modal instead of cart
	if (!logged.value) {
		uiStore.openLogin('login')
		return
	}
...
function onCheckout(payload?: any) {
	// cerrar panel del carrito y abrir panel de finalización
	showCart.value = false
	// guardar payload para que FinishedPanel lo use
	pendingOrder.value = payload ?? null
	showFinished.value = true
}

function onLogout() {
	// Cuando el usuario cierra sesión, además de cerrar el popover, limpiamos el carrito
	showProfile.value = false
	try {
		cart.clearCart()
	} catch (e) {
